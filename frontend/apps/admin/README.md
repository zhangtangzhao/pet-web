...
| `/products` | 商品管理 | 商品 CRUD、上下架、锁定 / 售出状态；表单含供货商 Select / 检疫证明 URL / 疫苗与驱虫到期日 |
| `/catalog` | 分类与品种 | 分类 / 品种 CRUD |
| `/orders` | 订单管理 | 详情（含优惠 / 会员折扣 / 定金明细 + 配送信息 + 健康保障天数）、退款、托运发货（运单号必填）/ 标记送达 / 打印托运单、导出 CSV |
...
| `/reviews` | 评价管理 | 全量评价（游标分页）；显示 / 隐藏 Switch、删除违规评价、官方回复（可覆盖更新，用户端评价区展示） |
| `/members` | 会员管理 | 搜索、等级列（等级 Tag + 成长值）、启用 / 禁用、会员导出 CSV |
...
| `/ops` | 审计 / 敏感词 | 管理端操作审计日志（非读操作自动落库）、敏感词库管理（聊天拦截 + 评价/售后打码） |
