...
| 页面 | 路径 | 说明 |
| ---- | ---- | ---- |
| 首页 | `pages/index` | 假搜索框跳列表、banner 运营位（`/api/home` 数据驱动按 jump_type 跳转）、分类导航跳列表、限时秒杀专区（秒杀价 + 剩余名额）、拼团专区（N人团角标直达拼团 checkout）、最近看过横滑（浏览历史 ≤10）、精选商品；分享卡片带邀请码参数 |
| 商品列表 | `pages/list` | 搜索框聚焦弹搜索面板（联想下拉 + 搜索历史 chips + 热搜榜 Top10）+ 分类 chips + 排序（默认/销量/价格升降），触底翻页 + 下拉刷新 |
| 我的 | `pages/me` | 头像昵称 / 手机号打码 / 注册时间 + 会员等级卡（等级徽章 / 成长值进度条 / 折扣）+ 入口列表（订单/评价/售后/优惠券/收藏/购物车/宠物档案/消息/智能选宠/客服）+ 退出登录 |
| 我的评价 | `pages/my-reviews` | 游标分页；含商家回复与平台隐藏标记 |
| 我的售后 | `pages/my-aftersales` | 分页列表（状态胶囊 / 金额 / 原因 / 平台备注），点击进售后进度页 |
| 详情 | `pages/detail` | 图片轮播 / 视频卡 / 宠物档案 / **规格选择 chips**（规格价切换）/ 详情长图纵览、AI 问宠、用户评价区（评分摘要 + 全部评价半屏弹层 + 商家回复）、看了又看横滑推荐、分享卡片带邀请码、🛒加入购物车 + 立即购买 |
| 确认下单 | `pages/checkout` | 三种模式：单商品 / 购物车批量（多商品一单）/ 拼团（拼团价 + N人团横幅）；增值服务多选、配送方式单选（自提免填地址 / 托运选地址簿 chips 或手填）、优惠券、定金锁宠开关（禁用券，展示定金 + 尾款）、秒杀价与规格价自动生效、金额明细（含运费与会员折扣行，与后端公式一致） |
...
| 订单详情 | `pages/order-detail` | 状态时间线（下单→支付→发货→送达→完成，自提单跳过发货/送达）、待支付 1s 倒计时、待补尾款紫色状态卡、**自提核销码大字卡（点击复制）**、定金明细、会员优惠行、配送信息与售后进度卡、健康证书入口、全套订单操作（演示环境无支付参数自动走 mock-pay） |
| 电子健康证书 | `pages/certificate` | 已完成订单的精美证书卡（证书编号 / 保障期 / 检疫证明预览），截图保存 |
| 订单评价 | `pages/order-review` | 星级 + 三维评分（健康/品相/服务）+ 文字 + 图片（COS 直传，≤9 张），一单一评 |
...
| 购物车 | `pages/cart` | 勾选结算 + 删除，底部合计 / 去结算（下架商品置灰不可勾选）；结算 ids 传递 checkout 批量下单 |
| 宠物档案 | `pages/pet` | 我的宠物档案（品种 / 生日 / 体重 / 疫苗驱虫到期日 / 备注）CRUD，到期自动推送护理提醒 |
