...
| 页面 | 路由 | 设计要点 |
| ---- | ---- | -------- |
| 首页 | `/index` | 沉浸式顶部（品牌色渐变）+ 假搜索框（跳列表页）+ banner 运营位（数据驱动，`/api/home` 下发按 `jump_type` 映射跳转）+ 分类快捷入口（跳列表页带 categoryId）+ 大图卡片瀑布流 |
...
| 确认订单 | `/checkout` | 商品卡 + 增值服务勾选 + 配送方式单选（名称/说明/运费，`/api/ship/methods`；托运 kind=2 追加收货地址 textarea 必填）+ 优惠券 + 费用明细（含运费行，`max(商品+服务-券, 0.01)+运费` 与后端一致）+ 联系人表单 + 支付按钮；提交成功弹窗直达订单详情 |
| 支付结果 | `/pay-result` | 成功动效 + 查看订单/返回首页 |
| 我的订单 | `/orders` | 状态 chips：全部/待支付/已支付/已完成/已退款；待支付卡显示剩余时间小字（单一定时器 30s 驱动，避免每卡秒级跳动）；按状态出操作——去支付、确认收货、申请售后、撤销售后、去评价/已评价；卡片点击进订单详情；小程序端在按钮点击内同步请求订阅消息授权（模板 ID 进页预取） |
| **订单详情** | `/order-detail?orderNo=` | 顶部状态卡（未支付含 1s 倒计时 `mm分ss秒`，到 0 提示已关单并刷新）+ 状态时间线（下单→支付→发货→送达→完成，有时间为实心品牌色点；自提单自动跳过发货/送达；已取消/关闭/退款末尾灰卡显示原因）+ 商品卡 + 金额明细 + 配送信息（方式/运费/地址/配送状态/运单号）+ 售后进度卡 + 操作区（复用 `orderActions.ts`：取消/去支付/确认收货/申请售后/撤销售后/去评价） |
...
| 订单管理 | `/orders` | ProTable（状态 Tab、退款操作二次确认、导出）；详情 Drawer 增加配送信息（方式/运费/地址/配送状态/运单号/发货送达时间），已支付+待配送出「发货」弹窗（运单号必填）、配送中出「标记送达」Popconfirm |
