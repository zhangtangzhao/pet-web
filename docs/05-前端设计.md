...
| 页面 | 路由 | 设计要点 |
| ---- | ---- | -------- |
| 首页 | `/index` | 沉浸式顶部（品牌色渐变）+ 假搜索框（跳列表页）+ banner 运营位（数据驱动，`/api/home` 下发按 `jump_type` 映射跳转）+ 分类快捷入口（跳列表页带 categoryId）+ **限时秒杀横滑专区**（`/api/flash-sales`，有生效活动才出现：秒杀价红色 + 剩余名额）+ **最近看过**（`/api/history/views` 横滑 ≤10，空隐藏）+ 大图卡片瀑布流；分享卡片带 `inviteCode` 参数（落地即缓存，登录归因）；落地带 inviteCode 参数同样缓存 |
...
| 我的 | `/me` | 头像昵称卡（手机号打码 + 注册时间）+ **会员等级卡**（`/api/level`：等级徽章 + 成长值 + 折扣 + 升级进度条，距下一级提示）+ 入口列表（订单/评价/售后/优惠券/收藏/消息/智能选宠/客服）+ 退出登录（确认后吊销 refreshToken 并清本地） |
...
| **商品详情** | `/detail/:id` | **核心页**：视频/头图轮播（自动播放静音视频，点击出声）→ 价格卡 → 宠物档案信息卡（性别/月龄/疫苗/驱虫图标化）→ 品种介绍卡 → 图文详情 → **看了又看**（`/api/products/:id/related` 横滑 4 只，redirectTo 换目标）→ 底部操作栏（收藏♥ / 联系客服 / 立即购买）；档案卡下方为**用户评价区**（均分 + 最新 3 条 + 「全部评价」半屏弹层，游标加载、图片预览、商家回复展示）；分享卡片携带 `?inviteCode=`（取 /invite 预取缓存） |
| 确认订单 | `/checkout` | 商品卡（命中秒杀自动切秒杀价 + 「秒杀」角标 + 原价划线）+ 增值服务勾选 + 配送方式单选（`/api/ship/methods`；托运 kind=2 追加地址簿 chips（`/api/addresses`，默认地址预选）或手填地址 textarea）+ **定金锁宠开关**（`/api/trade-config` 取比例，定金=商品+服务的 N%，开启后禁用优惠券）+ 优惠券 + 费用明细（含运费行与**会员折扣行**（trade-config 下发 myLevelName/myDiscount 时按 `round2(券后×折扣率)` 本地预览），`max(商品+服务-券, 0.01)×等级折扣 + 运费` 与后端一致）+ 联系人表单 + 支付按钮（定金模式显示「定金 + 尾款」双金额与「付定金锁宠」按钮）；提交成功弹窗直达订单详情 |
...
| **订单详情** | `/order-detail?orderNo=` | 顶部状态卡（未支付含 1s 倒计时 `mm分ss秒`，到 0 提示已关单并刷新；**待补尾款紫色状态卡**显示尾款截止时间）+ 状态时间线（下单→支付→发货→送达→完成，有时间为实心品牌色点；自提单自动跳过发货/送达；已取消/关闭/退款末尾灰卡显示原因）+ 商品卡 + 金额明细（定金单显示「已付定金」行，status=15 主行切「待补尾款」；**会员优惠行** `levelDiscount>0` 时显示）+ 配送信息（方式/运费/地址/配送状态/运单号）+ 售后进度卡 + **健康证书入口卡**（status=30：🩺 电子健康证书，点击进证书页）+ 操作区（复用 `orderActions.ts`：取消/去支付/取消并退定金/补尾款/确认收货/申请售后/撤销售后/去评价；演示环境无支付参数时自动走 mock-pay，生产 404 回退提示） |
...
| 宠物管理 | `/products` | ProTable 列表（缩略图列、状态 Tag、行内上下架）+ Drawer/Modal 表单（含**供货商 Select / 检疫证明 URL / 疫苗与驱虫到期日**，迁移 016） |
| 商品编辑 | `/products/edit` | 分组表单：基础信息 / 宠物档案 / 媒体上传（图片墙+视频上传，COS 直传）/ 富文本详情（wangEditor） |
| 分类品种 | `/categories`、`/breeds` | 表格 CRUD，品种页带分类级联 |
| 订单管理 | `/orders` | ProTable（状态 Tab、退款操作二次确认、**导出 CSV 按钮**（blob 下载，文件名带日期））；详情 Drawer 增加配送信息（方式/运费/地址/配送状态/运单号/发货送达时间）、定金信息（已付定金/待补尾款/健康保障天数）与**会员折扣行**（levelDiscount>0），已支付+待配送出「发货」弹窗（运单号必填）、配送中出「标记送达」Popconfirm、托运单出「打印托运单」（弹新窗口打印页：订单/活体/收发信息 + 保障期说明） |
...
| 会员管理 | `/members` | 列表（**等级列**：等级 Tag + 成长值）+ 禁用操作 + **会员导出 CSV** |
