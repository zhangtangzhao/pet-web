...
| 页面 | 路由 | 设计要点 |
| ---- | ---- | -------- |
| 首页 | `/index` | 沉浸式顶部（品牌色渐变）+ 假搜索框（跳列表页）+ banner 运营位（数据驱动，`/api/home` 下发按 `jump_type` 映射跳转）+ 分类快捷入口（跳列表页带 categoryId）+ **限时秒杀横滑专区**（`/api/flash-sales`，有生效活动才出现：秒杀价红色 + 剩余名额）+ **最近看过**（`/api/history/views` 横滑 ≤10，空隐藏）+ **拼团专区**（`/api/group-buys`：拼团价 + N人团角标，点击直达拼团 checkout）+ 大图卡片瀑布流；分享卡片带 `inviteCode` 参数（落地即缓存，登录归因）；落地带 inviteCode 参数同样缓存 |
| 商品列表 | `/list` | 搜索框（keyword，**聚焦弹搜索面板**：搜索联想 ≤10 条实时下拉 + 我的搜索历史 chips（单删/清空）+ 热搜榜 Top10（前三名高亮），`/api/search/hot|complete|history|trace`）+ 分类 chips + 排序 tabs（默认/销量/价格↑/价格↓）；触底翻页、下拉刷新；复用首页 pet-card 网格 |
| 我的 | `/me` | 头像昵称卡（手机号打码 + 注册时间）+ **会员等级卡**（`/api/level`：等级徽章 + 成长值 + 折扣 + 升级进度条，距下一级提示）+ 入口列表（订单/评价/售后/优惠券/收藏/**购物车**/**宠物档案**/消息/智能选宠/客服）+ 退出登录（确认后吊销 refreshToken 并清本地） |
...
| **商品详情** | `/detail/:id` | **核心页**：头图轮播 → 视频卡（`videoUrl` 有值展示）→ 价格卡（选中规格切规格价）→ **规格选择 chips**（`hasSku=1` 必选，未选点购买 toast 引导）→ 宠物档案信息卡（性别/月龄/疫苗/驱虫图标化）→ 品种介绍卡 → 图文详情 + **详情长图**（`detailImages` 纵向 widthFix，点击预览）→ **看了又看**（`/api/products/:id/related` 横滑 4 只，redirectTo 换目标）→ 底部操作栏（收藏♥ / 联系客服 / **🛒加购**（POST /cart，带 skuId）/ 立即购买）；档案卡下方为**用户评价区**（均分 + 最新 3 条 + 「全部评价」半屏弹层，游标加载、图片预览、商家回复展示）；分享卡片携带 `?inviteCode=`（取 /invite 预取缓存） |
| 确认订单 | `/checkout` | 三种模式：**单商品**（商品卡：命中秒杀自动切秒杀价 + 角标；规格价格随 `skuId` 切换）/ **购物车批量**（`?from=cart`，从 storage 取 `pet_cart_ids` 拉购物车清单多商品一单，隐藏服务/定金/券）/ **拼团**（`?groupId=`，拼团价 + N人团横幅，隐藏服务/定金/券）；增值服务勾选 + 配送方式单选（托运 kind=2 追加地址簿 chips 或手填地址 textarea）+ **定金锁宠开关** + 优惠券 + 费用明细（运费行 + 会员折扣行，与后端计价公式一致）+ 联系人表单 + 支付按钮；提交成功弹窗直达订单详情 |
...
| **订单详情** | `/order-detail?orderNo=` | 顶部状态卡（未支付含 1s 倒计时 `mm分ss秒`，到 0 提示已关单并刷新；**待补尾款紫色状态卡**显示尾款截止时间；**自提已支付单显示大字核销码**（点击复制））+ 状态时间线（下单→支付→发货→送达→完成，有时间为实心品牌色点；自提单自动跳过发货/送达；已取消/关闭/退款末尾灰卡显示原因）+ 商品卡（含规格 `skuSpecs`）+ 金额明细（定金单显示「已付定金」行，status=15 主行切「待补尾款」；**会员优惠行** `levelDiscount>0` 时显示）+ 配送信息（方式/运费/地址/配送状态/运单号）+ 售后进度卡 + **健康证书入口卡**（status=30：🩺 电子健康证书，点击进证书页）+ 操作区（复用 `orderActions.ts`：取消/去支付/取消并退定金/补尾款/确认收货/申请售后/撤销售后/去评价；演示环境无支付参数时自动走 mock-pay，生产 404 回退提示） |
| 电子健康证书 | `/certificate?orderNo=` | 品牌渐变精美卡片（🐾 纹章 + 证书编号 + 持有人打码 + 宠物/品种 + 完成时间 + 保障期 = 完成时间 + guaranteeDays；检疫证明 URL 可点开预览、供货商名称有值展示）；提示「截图保存」；41806 不可开具时 toast |
| 订单评价 | `/order-review?orderNo=` | 星级自绘 + **三维评分**（健康程度/品相外观/服务体验三行五角星）+ 文字（≤500 字）+ 图片墙（COS 直传，≤9 张），提交后返回订单页 |
...
| 登录 | `/login` | 微信一键登录（小程序）为主，可切换「手机号 + 验证码」；60s 倒计时发送按钮；测试/开发环境展示公共验证码提示；提交时自动附带缓存的 `inviteCode`（分享带参归因，仅新注册生效） |
...
| 购物车 | `/cart` | 勾选圆点 + 商品卡（图/题/规格胶囊/价/删除）；下架商品置灰不可勾选；底部固定栏（合计 + 去结算 N）→ 结算 ids 存 storage 跳 checkout `from=cart` |
| 宠物档案 | `/pet` | 档案卡（昵称/品种/性别徽章/生日体重/疫苗驱虫到期橙色胶囊/备注）+ 编辑/删除 + 内联新增表单（昵称/品种/性别分段/生日/体重/三个日期/备注） |
...
| 宠物管理 | `/products` | ProTable 列表（缩略图列、状态 Tag、行内上下架）+ Drawer/Modal 表单（含**供货商 Select / 检疫证明 URL / 疫苗与驱虫到期日**（016）/**详情长图与库存预警阈值/SKU 规格编辑器**（018）） |
| 商品编辑 | `/products/edit` | 分组表单：基础信息 / 宠物档案 / 媒体上传（图片墙+视频上传，COS 直传）/ 富文本详情（wangEditor） |
| 分类品种 | `/categories`、`/breeds` | 表格 CRUD，品种页带分类级联 |
| 订单管理 | `/orders` | ProTable（状态 Tab、退款操作二次确认、**导出 CSV 按钮**（blob 下载，文件名带日期））；详情 Drawer 增加配送信息（方式/运费/地址/配送状态/运单号/发货送达时间）、定金信息（已付定金/待补尾款/健康保障天数）、**会员折扣行**（levelDiscount>0）与**自提核销码行**，已支付+待配送出「发货」弹窗（运单号必填）、配送中出「标记送达」Popconfirm、托运单出「打印托运单」（弹新窗口打印页）、**自提单出「自提核销」弹窗**（6 位码校验 20→30） |
...
| 拼团活动 | `/group-buys` | 表格（商品/拼团价/N人团/时限/启用 Switch/时间）+ Modal（商品远程搜索 Select、拼团价、成团人数 2-10、时限 1-72 小时）+ 停用/删除 Popconfirm；说明「超时未成团自动退款」 |
...
| 评价管理 | `/reviews` | 游标加载列表（星级 / **三维分列**（健康/品相/服务）/ 内容 / 图片预览 / 会员 / 商品 / 官方回复列）+ 显示隐藏 Switch + 「回复」弹窗（TextArea ≤200 字，回填已有回复、可覆盖更新）+ 删除 Popconfirm |
| 会员管理 | `/members` | 列表（**等级列**：等级 Tag + 成长值；**黑名单列**）+ 禁用 Switch + **拉黑/解除** Popconfirm（可登录禁交易）+ **会员导出 CSV** |
| 财务对账 | `/finance` | Tabs 双页：**交易流水**（订单号搜索 + 实付/退款/支付方式/支付单号/状态/时间，分页）+ **每日汇总**（7/14/30/60/90 天切换：成交单数/支付/退款/净收入）；页头为**导出三连**（订单/会员/积分 CSV，blob 下载） |
| 供货商管理 | `/suppliers` | 表格（名称/联系人/电话/地址/在售商品数/启用 Switch/时间）+ 关键词搜索 + Modal 表单（名称必填 ≤64）+ 删除 Popconfirm（被商品引用后端 41901 提示） |
| 经营报表 | `/report` | 汇总 Statistic ×4（今日 GMV/订单、本月 GMV/订单、待处理售后）+ 近 N 天成交表（占比进度条，7/14/30/60/90 天切换）+ 商品销量 Top10（Progress 条）+ 品类成交分布双列 |
| 审计/敏感词 | `/ops` | Tabs 三页：操作审计表（管理员/方法 Tag/路径 code/IP/时间，分页）+ 敏感词管理（回车/按钮添加 ≤64 字、启用 Switch、删除 Popconfirm）+ **风控记录**（规则 Tag：下单频控/评价联系方式/黑名单 + 会员ID + 详情 + 时间，规则筛选，分页） |
...
- 路由守卫：无 token / token 过期跳登录页；多角色 RBAC 后端拦截（operator 禁退款/会员写，support 白名单），前端 403 统一 toast 提示（菜单不按角色裁剪，操作由服务端兜底）。
