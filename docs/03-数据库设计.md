...
### 3.20 通知投递队列 `notification`（微信订阅/模板消息，迁移 008）
...
> 投递器每 10s 扫描，单行事务 `FOR UPDATE SKIP LOCKED` 取件（多实例不重复投递）；小程序订阅消息优先、公众号模板消息兜底；模板未配置 → 置 3/0 降级（开发环境零配置可跑）。`biz_key` 唯一是最终幂等兜底。
...
| notification | 短生命周期（事件驱动） | `(id) WHERE status=0` 部分索引供投递器扫描，biz_key 唯一幂等 |
...
- `order_review` / `after_sale` / `notification`（006-008）：无种子数据。
